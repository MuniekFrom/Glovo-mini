...
public class TopBarController {
    ...
    private Popup buildKontoPopup() {
        ...
        ordersPopup = buildOrdersPopup();
        showPopupUnderNode(kontoLabel, ordersPopup);
        });
    ...
    private Popup buildLoginPopup() {
        ...
        btnLogin.setOnAction(e -> {
            ...
            try {
            Integer id = userRepo.loginGetUserId(l, p); 
    ...
    private Popup buildRegisterPopup() {
        ...
        private Popup buildOrdersPopup() {
        Label title = new Label("Moje zamówienia");
        title.setStyle("-fx-font-weight: bold;");

        VBox listBox = new VBox(6);

        try {
            var ordersList = orderRepo.getOrdersForUser(Session.getUserId());

            if (ordersList.isEmpty()) {
                listBox.getChildren().add(new Label("Brak zamówień."));
            } else {
                for (int i = 0; i < ordersList.size(); i++) {
                    Button item = new Button((i + 1) + ". " + ordersList.get(i));
                    ...
                    listBox.getChildren().add(item);
                        ...
                        Label err = new Label("Błąd pobierania zamówień");
        ...
        close.setOnAction(e -> hideAllPopups());

        VBox box = new VBox(10, title, listBox, close);
        stylePopupBox(box);
    ...
    private void hideAllPopups() {
        ...
        if (ordersPopup != null) ordersPopup.hide();
    }
